...
const TOKEN_FILES_PATH = resolve(
...
describe("Token Loading Integration", () => {
	...
	it("should have refresh_token field in each token", () => {
		for (const [email, token] of tokens) {
			expect(token.refresh_token, `${email} missing refresh_token`).toBeDefined();
			expect(
				token.refresh_token,
	...
	it("should have access_token field in each token", () => {
		for (const [email, token] of tokens) {
			...
			expect(
				token.access_token,
				`${email} access_token is empty`,
			).toBeTruthy();
			expect(typeof token.access_token, `${email} access_token not string`).toBe(
				"string",
			);
		}
	});
