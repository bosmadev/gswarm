...
function detectBumpType(msg: string): "major" | "minor" | "patch" {
	...
	if (/^feat(\(.+\))?:/.test(lowerMsg) || msg.includes("### Added"))
		return "minor";
	return "patch";
}
...
function discoverNewCommits(): CommitInfo[] {
	...
	console.log(
		`Commit discovery range: ${range}${lastBotSha ? ` (since ${lastBotSha.slice(0, 8)})` : " (no bot commits found)"}`,
	);
	...
	for (const record of records) {
		...
		if (/^(chore|docs):\s*(bump to v|update CHANGELOG)/i.test(subject))
			continue;
...
function extractCommitContent(commit: CommitInfo): {
	summary: string;
	changes: string[];
	...
	if (commitsIdx !== -1) {
		...
		for (const line of rawLines) {
			...
			if (
				...
				joinedLines[joinedLines.length - 1].trim().startsWith("-")
			) {
		...
		// Preserve ### category headings (Added, Fixed, Changed, etc.)
		// Group bullets under their heading; uncategorized bullets go to flat list
		let currentHeading = "";
		for (const line of joinedLines) {
			const trimmed = line.trim();
			if (trimmed.startsWith("### ")) {
				currentHeading = trimmed.replace(/^###\s+/, "").trim();
				if (!categorized[currentHeading]) {
					categorized[currentHeading] = [];
				}
				...
				if (currentHeading && categorized[currentHeading]) {
					categorized[currentHeading].push(processed);
				} else {
					changes.push(processed);
				}
			}
		}
	}
	...
	if (detailsIdx !== -1) {
		let endIdx = lines.findIndex(
			(l, i) => i > detailsIdx && l.startsWith("## "),
		);
		if (endIdx === -1) endIdx = lines.length;
		const detailLines = lines
			.slice(detailsIdx + 1, endIdx)
			.filter((l) => l.trim().startsWith("-"));
		// Collect all existing bullets for dedup (from both categorized and flat)
		const allBullets = [...changes];
		for (const bullets of Object.values(categorized)) {
			allBullets.push(...bullets);
		}
		const existingSet = new Set(allBullets.map((c) => c.trim().toLowerCase()));
	...
	if (!summary && changes.length === 0 && !hasCategorized) {
	...
	// Filter trivia from both flat and categorized changes
	changes = changes.filter((l) => !isTrivia(l));
	for (const heading of Object.keys(categorized)) {
		categorized[heading] = categorized[heading].filter((l) => !isTrivia(l));
		if (categorized[heading].length === 0) {
			delete categorized[heading];
		}
	}

	return { summary, changes, categorized };
}
...
function buildChangelogEntry(
	...
	const { summary, changes, categorized } = extractCommitContent(commit);

	const toCheckbox = (l: string) =>
		/^- \[x\]/.test(l) ? l : l.replace(/^- /, "- [x] ");

	let entry = `---\n\n## ${badge} | Build ${buildId}\n\n`;
	if (summary) entry += `${summary}\n\n`;

	// Emit categorized changes with ### headings (Added, Fixed, Changed, etc.)
	const headingOrder = ["Added", "Fixed", "Changed", "Removed", "Updated"];
	const emittedHeadings = new Set<string>();
	for (const heading of headingOrder) {
		if (categorized[heading] && categorized[heading].length > 0) {
			entry += `### ${heading}\n`;
			entry += `${categorized[heading].map(toCheckbox).join("\n")}\n`;
			emittedHeadings.add(heading);
		}
	}
	// Emit any remaining headings not in the standard order
	for (const heading of Object.keys(categorized)) {
		if (!emittedHeadings.has(heading) && categorized[heading].length > 0) {
			entry += `### ${heading}\n`;
			entry += `${categorized[heading].map(toCheckbox).join("\n")}\n`;
		}
	}

	// Emit uncategorized/flat changes (Details section, old-format fallback)
	if (changes.length > 0) {
		entry += `${changes.map(toCheckbox).join("\n")}\n`;
	}
...
for (const commit of commits) {
	...
	if (commit.buildId) {
		...
		if (explicitId <= highestBuild) {
			...
			console.log(
				`⚠ Stale Build ID ${commit.buildId} in commit (highest is ${highestBuild - 1}), reassigned to Build ${buildId}: ${commit.subject.slice(0, 60)}`,
			);
		} else {
		...
		buildId = String(highestBuild);
		console.log(
			`Auto-assigned Build ${buildId} to: ${commit.subject.slice(0, 60)}`,
		);
	}
	...
	if (duplicateCheck.test(changelog)) {
		console.log(
			`Build ${buildId} already in CHANGELOG, skipping: ${commit.subject.slice(0, 60)}`,
		);
		continue;
	}
	...
	else if (bt === "minor" && combinedBumpType !== "major")
		combinedBumpType = "minor";
	...
	console.log(
		`✓ Added CHANGELOG entry for Build ${buildId}: ${commit.subject.slice(0, 60)}`,
	);
}
...
if (pkgPath && fs.existsSync(pkgPath)) {
	...
	console.log(
		`${combinedBumpType.toUpperCase()} bump: ${currentVersion} → ${newVersion}`,
	);
...
console.log(
	`✓ Updated CHANGELOG.md for Build(s) ${processedBuildIds.join(", ")}`,
);
